...
rm(ids_bajas, DT_continua, ids_continua)
```
...
DT_bajas_hist[, foto_mes_baja2 := 202105]
DT_bajas_hist[mapeo_baja, foto_mes_baja2 := mapeo_baja$foto_mes_baja2, on="numero_de_cliente"]
...
DT_bajas_hist[mapeo_baja, cluster := mapeo_baja$cluster, on="numero_de_cliente"]
rm(mapeo_baja)
```
...
DT_bajas_hist = setnafill(DT_bajas_hist, 
                          type = "const", 
                          fill = -1, 
                          nan=NA, 
                          cols = cols_con_na)
...
```{r}
DT3[mapeo_clusters, cluster := mapeo_clusters$cluster, on="numero_de_cliente"]
```
