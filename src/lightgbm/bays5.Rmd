...
```{r}
dataset[, internet := fifelse(internet > 1, 1, 0) ]
```
...
dataset[, (cols_media) := frollmean(.SD, n=(n), fill=NA, align="right", algo="fast"), 
                      .SDcols = (cols_a_usar), by=numero_de_cliente]
...
for(col in cols_a_usar)
{
  cols_dif1 = c(cols_dif1, paste0(col, "_dif_1"))
  cols_lag1 = c(cols_lag1, paste0(col, "_lag_1"))
}
for (i in 1:length(cols_a_usar))
{
  dataset[, (cols_dif1[i]) := dataset[[cols_a_usar[i]]] - dataset[[cols_lag1[i]]] ]
}

#----- media dif_1    n=3
n <- 3L
cols_media_dif1 = c()
for(col in cols_dif1)
{
  cols_media_dif1 = c(cols_media_dif1, paste0(col, "_media_", n))
}

dataset[, (cols_media_dif1) := frollmean(.SD, n=(n), fill=NA, align="right", algo="fast"), 
                      .SDcols = (cols_dif1), by=numero_de_cliente]

dataset[, (cols_media_dif1) := shift(.SD, n=1L, fill=NA, type="lag"),
                      .SDcols = (cols_media_dif1), by=numero_de_cliente] 


#----- dif_2+
cols_dif_prev = cols_dif1
for (i in 2:2)
{
  cols_dif = c()
  for(col in cols_a_usar)
  {
    cols_dif = c(cols_dif, paste0(col, "_dif_", i))
  }
...
rm(cols_a_usar, cols_lag, cols_dif, cols_dif_prev, 
   cols_dif1, cols_lag1, cols_media, cols_media_dif1)
```
...
if (FALSE)
{
  ...
  if (FALSE) # rango total
  {
    ...
    dataset[, (nuevas_cols) := frank(.SD),
            .SDcols = (cols_nrm_rango), by=foto_mes]
  }
  else { # rango relativo a 0
    for(col in cols_nrm_rango)
    {
      nueva_col = paste0(col, "_nrmr0")
      dataset[get(col) > 0, (nueva_col) := frank(get(col)), by=foto_mes]
      dataset[get(col) < 0, (nueva_col) := -frankv(get(col), order=-1L), by=foto_mes]
      dataset[get(col) == 0, (nueva_col) := 0]
    }
  }
  
  # remover cols originales
  if (TRUE) 
  {
    for(col in cols_nrm_rango)
    {
