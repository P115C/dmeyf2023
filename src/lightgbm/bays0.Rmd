...
if (TRUE)
{
  ...
  dataset[, (nuevas_cols) := lapply(.SD, rollmean, k=(n), fill=NA, align="right"),  
                    .SDcols = (campos_media), by=numero_de_cliente]
  dataset[, (nuevas_cols) := lapply(.SD, shift, n=1L, fill=NA, type="lag"),  
  ...
  dataset[, (nuevas_cols) := lapply(.SD, function(x) c(NA, diff(x, lag=1, differences=1))),
                      .SDcols = (campos_media), by=numero_de_cliente]
  ...
  dataset[, (nuevas_cols) := lapply(.SD, function(x) {
...
} else
{
  ...
  for(col in campos_media)
  {
    ...
    if (FALSE)
    {
      ...
      nuevas_cols = c(nuevas_cols, nueva_col)
      dataset[, (nueva_col) := {
            shift(.SD[[col]], 1L, fill = NA, type="lag")
        }, .(numero_de_cliente)]
    }
...
if (TRUE)
{
  ...
  dataset[, (nuevas_cols) := lapply(.SD, function(x) round(rank(x)/.N, 5)),
...
} else
{
  for (col in campos_nrm_rango)
  {
    ...
    dataset[, (nueva_col) := round(rank(.SD[[col]])/.N, 5), by = "foto_mes"]
    dataset[, (col) := NULL]
  }
}
```
