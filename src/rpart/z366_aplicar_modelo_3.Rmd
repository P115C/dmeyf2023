...
PARAM$rpart$cp <- -1
PARAM$rpart$minsplit <- 1185 # 1733
PARAM$rpart$minbucket <- 192  # 378
PARAM$rpart$maxdepth <- 10 # 12

PARAM$semilla_azar <- c(886609, 201821, 623423, 105389, 151051)
PARAM$xval_folds <- 5  # folds del cross validation

archivo_log <- "aplicar_modelo_log0_bin2.txt"
#-----------------------------
...
for (semilla in PARAM$semilla_azar)
{
  ...
  for (i in seq(PARAM$xval_folds))  # 1 2 3 4 5
  {
      ...
      # esta es la probabilidad de POS
      prob_pos <- prediccion[, "POS"]
      
      tb_final <- copy(dataset[fold == i, list(clase_ternaria)])
      tb_final[, prob := prob_pos]
      setorder(tb_final, -prob)
      
      # calculo la ganancia
      g <- tb_final[
        1:round(PARAM$corte / PARAM$xval_folds),
