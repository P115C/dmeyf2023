...
PARAM$finalmodel$optim$num_iterations <- 966
PARAM$finalmodel$optim$learning_rate <- 0.055232
PARAM$finalmodel$optim$feature_fraction <- 0.226426
PARAM$finalmodel$optim$min_data_in_leaf <- 43059
PARAM$finalmodel$optim$num_leaves <- 301		
...
dataset[, intra1 := fcase( get("ctarjeta_t_transacciones") != 0, get("t_madelantodolares") / get("ctarjeta_t_transacciones"),
                           default = NA ) ]
dataset[, intra2 := get("mactivos_margen") + get("mpasivos_margen")]
dataset[, intra3 := get("mcaja_ahorro") + get("mcaja_ahorro_adicional") + get("mcaja_ahorro_dolares")]
dataset[, intra4 := get("mprestamos_personales") + get("mprestamos_hipotecarios") + get("mprestamos_prendarios")]
dataset[, intra5 := get("mpayroll") + get("mpayroll2")]
dataset[, intra6 := fcase( get("intra5") != 0, get("intra4") / get("intra5"),
                           default = NA ) ]
...
cols_pesos <- c("intra1", "intra2", "intra3", "intra4", "intra5",
                "mcuentas_saldo", "mprestamos_personales", "mcuenta_corriente_adicional",
